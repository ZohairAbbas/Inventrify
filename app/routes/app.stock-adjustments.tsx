...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  // The form's product picker searches on demand. Only the preselected product — the one
  // ?product= names — is resolved up front, so its current stock can seed the
  // "current → new" preview before any search happens.
  const preselected = filteredProduct
    ? await prisma.product.findFirst({
        where: { id: filteredProduct.id, shop },
        select: { id: true, title: true, variantTitle: true, currentStock: true },
      })
    : null;
  ...
  return {
    preselected: preselected
      ? {
          id: preselected.id,
          label: preselected.variantTitle
            ? `${preselected.title} — ${preselected.variantTitle}`
            : preselected.title,
          currentStock: preselected.currentStock,
        }
      : null,
    adjustments,
    reversedIds,
...
export default function StockAdjustments() {
  const {
    preselected, adjustments, reversedIds, locations, filteredProduct, unknownProductFilter,
  ...
  // adjust — so preselect it rather than making the merchant search for it again.
  const [product, setProduct] = useState<{ id: string; label: string; currentStock: number } | null>(
    preselected,
  );
      ...
      <div style={{ maxWidth: "var(--inv-content-max)", margin: "0 auto", padding: "22px var(--inv-gutter) 80px" }}>
        ...
        <Card style={{ marginBottom: "18px" }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-end", gap: "24px", marginTop: "20px", flexWrap: "wrap" }}>
            ...
            {product && (
              <div style={{ display: "flex", alignItems: "center", gap: "12px", fontSize: "14px", color: "var(--inv-text-2)", paddingBottom: "8px" }}>
                <div>
                  Current <b style={{ fontFamily: "var(--inv-font-mono)", color: "var(--inv-ink)" }}>{product.currentStock}</b>
                </div>
                <span>→</span>
                <div>
          ...
          <div style={{ marginTop: "22px" }}>
            <Button
              variant="primary"
              disabled={isBusy || !delta || !product}
              onClick={() =>
                fetcher.submit(
                  { productId: product?.id ?? "", delta: String(delta), reason, note, locationId },
                  { method: "POST" },
                )
              }
            >
